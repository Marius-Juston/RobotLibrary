...
public class Kinematics {
	...
	private void evaluateNextPose(final double ds) {
		...
		case LimitLinearAcceleration:
			...
			final double vAccel = Math.sqrt(v0 * v0 + motion.aMax * Math.abs(lMidpoint));
			final double vDecel = Math.sqrt(v1 * v1 + motion.aMax * Math.abs(motion.getLength() - lMidpoint));
	...
	public KinematicPose interpolatePose(final double t) {
		...
		final double dt = nextPose.t - lastPose.t;
		final double p = (nextPose.t - t) / dt;
		final double q = (t - lastPose.t) / dt;
